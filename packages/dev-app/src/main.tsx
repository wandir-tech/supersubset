...
import { SupersubsetRenderer, createWidgetRegistry, type WidgetProps, type NavigateRequest } from "@supersubset/runtime";
import { registerAllCharts } from "@supersubset/charts-echarts";
import { resolveTheme, themeToCssVariables } from "@supersubset/theme";
import { SupersubsetDesigner, ImportExportPanel, CodeViewPanel, LivePreviewPane, useUndoRedo, UndoRedoToolbar, useUndoRedoKeyboard, FilterBuilderPanel, SlideOverPanel, InteractionEditorPanel } from "@supersubset/designer";
 ...
 */
const DEMO_DATA: Record<string, { data: Record<string, unknown>[]; columns?: WidgetProps["columns"] }> = {
  // Overview page
...
function App() {
  ...
  const [activeDashboardId, setActiveDashboardId] = useState(dashboardSwitchingDemo.initialDashboardId);
  ...
  const [demoInteractions, setDemoInteractions] = useState<import('@supersubset/designer').InteractionDefinition[]>([]);
  ...
  const demoDatasets: NormalizedDataset[] = useMemo(() => [
    {
      ...
      label: 'Orders',
      fields: [
        { id: 'order_date', label: 'Order Date', dataType: 'date' as const, role: 'time' as const },
        { id: 'month', label: 'Month', dataType: 'string' as const, role: 'time' as const },
        { id: 'category', label: 'Category', dataType: 'string' as const, role: 'dimension' as const },
        { id: 'region', label: 'Region', dataType: 'string' as const, role: 'dimension' as const },
        { id: 'product', label: 'Product', dataType: 'string' as const, role: 'dimension' as const },
        ...
        { id: 'source', label: 'Source', dataType: 'string' as const, role: 'dimension' as const },
        ...
        { id: 'revenue', label: 'Revenue', dataType: 'number' as const, role: 'measure' as const, defaultAggregation: 'sum' as const },
        { id: 'prevRevenue', label: 'Prev Revenue', dataType: 'number' as const, role: 'measure' as const },
        { id: 'orders', label: 'Orders', dataType: 'integer' as const, role: 'measure' as const, defaultAggregation: 'count' as const },
        { id: 'prevOrders', label: 'Prev Orders', dataType: 'integer' as const, role: 'measure' as const },
        { id: 'aov', label: 'Avg Order Value', dataType: 'number' as const, role: 'measure' as const },
        { id: 'quantity', label: 'Quantity', dataType: 'integer' as const, role: 'measure' as const, defaultAggregation: 'sum' as const },
        { id: 'amount', label: 'Amount', dataType: 'number' as const, role: 'measure' as const, defaultAggregation: 'sum' as const },
        ...
        { id: 'margin', label: 'Margin %', dataType: 'number' as const, role: 'measure' as const },
        { id: 'achievement', label: 'Achievement', dataType: 'number' as const, role: 'measure' as const },
        { id: 'count', label: 'Count', dataType: 'integer' as const, role: 'measure' as const, defaultAggregation: 'count' as const },
        { id: 'sales', label: 'Sales', dataType: 'number' as const, role: 'measure' as const, defaultAggregation: 'sum' as const },
        { id: 'visits', label: 'Visits', dataType: 'integer' as const, role: 'measure' as const, defaultAggregation: 'sum' as const },
      ],
    ...
    },
    {
      ...
      fields: [
        { id: 'alert_title', label: 'Alert Title', dataType: 'string' as const, role: 'dimension' as const },
        { id: 'alert_message', label: 'Alert Message', dataType: 'string' as const, role: 'dimension' as const },
        { id: 'severity', label: 'Severity', dataType: 'string' as const, role: 'dimension' as const },
        { id: 'detected_at', label: 'Detected At', dataType: 'date' as const, role: 'time' as const },
      ],
  ...
  const viewerDashboard = useMemo(() => {
    ...
    if (viewerScenario === 'dashboards') {
      return dashboardSwitchingDemo.dashboards.find((dashboard) => dashboard.id === activeDashboardId)
  ...
  const [viewerInitialFilterValues, setViewerInitialFilterValues] = useState<Record<string, unknown> | undefined>(undefined);
  ...
  const designerResolvedTheme = useMemo(() => resolveTheme(designerInlineTheme ?? null), [designerInlineTheme]);
  const designerCssVars = useMemo(() => themeToCssVariables(designerResolvedTheme), [designerResolvedTheme]);
  const viewerInlineTheme = viewerDashboard.theme as InlineThemeDefinition | undefined;
  const viewerResolvedTheme = useMemo(() => resolveTheme(viewerInlineTheme ?? null), [viewerInlineTheme]);
  const viewerCssVars = useMemo(() => themeToCssVariables(viewerResolvedTheme), [viewerResolvedTheme]);

  const handleViewerNavigate = useCallback((request: NavigateRequest) => {
    if (request.target.kind !== 'page') {
      console.warn('[Supersubset] Dashboard navigation targets are not enabled in the dev app yet.', request.target);
      return;
    }
  ...
  const handlePublish = useCallback((dashboard: DashboardDefinition) => {
    ...
    alert(`Dashboard "${dashboard.title}" saved! (${dashboard.pages[0]?.widgets?.length ?? 0} widgets)`);
  }, [undoRedo]);
      ...
      <div style={{
        ...
        {savedDashboard && (
          <span style={{ marginLeft: 'auto', fontSize: 12, color: '#aaa' }}>
            Last saved: {savedDashboard.title} ({savedDashboard.pages[0]?.widgets?.length ?? 0} widgets)
          </span>
        )}
      </div>
      ...
      {mode === 'designer' ? (
        <div style={{ display: 'flex', height: 'calc(100vh - 44px)' }}>
          <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
            <div style={{ flex: showCode ? '1 1 60%' : '1 1 100%', overflow: 'hidden', minHeight: 0 }}>
            <SupersubsetDesigner
              ...
              headerActions={
                ...
                >
                  ...
                  <button
                    onClick={() => { setShowFilters(!showFilters); setShowInteractions(false); }}
                  ...
                  <button
                    onClick={() => { setShowInteractions(!showInteractions); setShowFilters(false); }}
                  ...
                  >
                    ⚡ Interactions{demoInteractions.length > 0 ? ` (${demoInteractions.length})` : ''}
                  </button>
                </div>
              }
            />
        ...
        <div style={{ maxWidth: 1400, margin: "0 auto", padding: "24px" }}>
          ...
          >
            <div style={{ display: 'flex', gap: 16, justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap' }}>
              <div style={{ maxWidth: 760 }}>
                ...
                <p style={{ margin: '10px 0 8px', color: '#334155', lineHeight: 1.55 }}>
                  A page is another canvas inside the same dashboard document. Pages share the dashboard id, filter contract, theme, and saved state. A separate dashboard is a different document with its own identity, route, defaults, and ownership boundary.
                </p>
                <p style={{ margin: 0, color: '#475569', lineHeight: 1.55 }}>
                  Use pages when you are staying inside one analytical workbook. Use separate dashboards when the user is moving into a different analytical product surface. The demos below show both patterns with the current runtime.
                </p>
              </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(260px, 1fr))', gap: 12, marginTop: 16 }}>
              <div style={{ background: 'rgba(255, 255, 255, 0.84)', borderRadius: 12, padding: 14, border: '1px solid rgba(148, 163, 184, 0.2)' }}>
                <strong style={{ display: 'block', marginBottom: 6, color: '#0f172a' }}>Page</strong>
                <span style={{ color: '#475569', lineHeight: 1.5 }}>
                  An alternate view inside one dashboard document. Same id, same filter bar, same interaction set, same persistence envelope.
                </span>
              </div>
              <div style={{ background: 'rgba(255, 255, 255, 0.84)', borderRadius: 12, padding: 14, border: '1px solid rgba(148, 163, 184, 0.2)' }}>
                <strong style={{ display: 'block', marginBottom: 6, color: '#0f172a' }}>Separate dashboard</strong>
                <span style={{ color: '#475569', lineHeight: 1.5 }}>
                  A different document. It can have a different title, route, layout, theme, owner, defaults, and filter contract.
                </span>
              </div>
              <div style={{ background: 'rgba(255, 255, 255, 0.84)', borderRadius: 12, padding: 14, border: '1px solid rgba(148, 163, 184, 0.2)' }}>
                <strong style={{ display: 'block', marginBottom: 6, color: '#0f172a' }}>What to try</strong>
                <span style={{ color: '#475569', lineHeight: 1.5 }}>
                  In the page demo, click the region chart to navigate to another page. In the dashboard demo, use the host buttons to switch documents and notice the separate titles and themes.
                </span>
              </div>
            </div>
          </section>
          ...
          {viewerScenario === 'dashboards' && (
            <section style={{ marginBottom: '16px', display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
              ...
              {viewerPages.map((page) => (
                <button
                  ...
                  style={{
                    ...
                    background: viewerActivePage === page.id ? 'var(--ss-color-primary, #1677ff)' : 'transparent',
